...
| `plate` | The device centred, whole and **never cropped**, presented like a plate in a catalogue: type as a caption above it and a second line below, generous constant margin on every edge. The discipline is margin rather than crop, which is what makes it the opposite of `type-over-device` despite the similar mass. Invented here for `bio`, 2026-08-30. Needs another axis to carry the set rhythm — five uncropped centred devices is `uniform` by default — and needs screens worth showing whole; it fails on a capture whose interest is in one corner. |
