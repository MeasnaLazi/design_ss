...
Browser-based editor for Store–style screenshot layouts. It runs as a **Vite + React** app with a **Fabric.js** canvas, persists designs under the repo **`datasource/`** folder (dev server only), and exposes a **loopback HTTP API** for `composer/import-to-canvas.mjs` and agents.
...
- **Import & agent integration** — While `npm run dev` is up, `composer/import-to-canvas.mjs` drives the open tab over **Server-Sent Events** to replay a rendered strip into the canvas as editable layers. See [TOOLKIT.md](./TOOLKIT.md) for the full API contract.

This UI is the **live editor** half of **apps_publisher**; offline layout helpers live under each skill's `script/` folder and the HTML renderer in **`composer/`**.
...
| **Port** | Default dev/preview port **4713** ([`vite.config.ts`](./vite.config.ts)). Ensure nothing else binds that port, or change the config (and `composer/import-to-canvas.mjs --api`) consistently. |
| **Import handoff (optional)** | The dev server exposes the `/__api/screenshot-designer/` endpoints that `composer/import-to-canvas.mjs` replays a rendered strip into (needs `composer/` npm deps). |

No separate `.env` is required for basic local UI use; optional Vite env vars (e.g. preview multiplier) are documented where relevant.
...
There is **no** `npm run clean` or scheduled cleanup — **only manual deletion** (Finder, `rm`, or your editor).
...
Open **http://localhost:4713** (or the URL Vite prints). Keep the tab open for the `import-to-canvas` handoff — the browser must subscribe to the command stream for your active artboard **slug**.
...
### Work with the composer import

1. Start **`npm run dev`** in `web_ui/`.
2. Open the designer in a browser tab on the preset you care about.
3. From the repo root, render a strip with `composer/render.mjs`, then replay it into the open tab with `node composer/import-to-canvas.mjs --strip … --preset …`.
