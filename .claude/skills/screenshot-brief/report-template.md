# Agent contract (screenshot-agent / screenshot-brief skill)
...
**Device frame pack:** Before filling **Device frame pack**, extract **`pack_id`** from that store JSON’s **`device_pack_path`** (folder after `device-frames`, e.g. `iphone_12_pro`), then run **`node .claude/skills/screenshot-brief/script/load-frame.mjs --pack <pack_id>`** from the publisher repo root. On success, for each panel row set **Device frame pack** to that row’s **`framePath`** from the CLI JSON (`frames` array, in list order—first frame for `order` 1, second for `order` 2, etc.; use `—` if the pack has fewer frames than panels). On failure use `—`.
