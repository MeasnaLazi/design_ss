...
5. **`screenshot_panel`** — Typography lock + **`creative_plan.panels`** in order; **per panel**, full-strip **`predict-checks --from-export`** must pass (bounded fix retries) before advancing; user on hard errors / retry exhaustion.
