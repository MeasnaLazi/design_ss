...
| `python toolkit/scripts/designer.py enqueue-op` | **`--operation <string>`** (required). **`--args-json`** JSON object or **`@file.json`** (default **`{}`**). Optional **`--request-id <string>`** (echoed in SSE). **`--timeout <float>`** (default **120**). | POST `/enqueue-command` (runs in open Web UI tab via SSE); toolkit validates args before HTTP |
