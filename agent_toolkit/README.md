...
python -m agent_toolkit designer pull-preview --panels "0,1" --out /tmp/seg.png   # contiguous columns only → one combined PNG
python -m agent_toolkit designer pull-export                        # after enqueue export_json (compact layout summary JSON)
python -m agent_toolkit designer pull-export --panels "0,1"        # adjacent columns only → per-panel summaries (panel-local coords)
```
